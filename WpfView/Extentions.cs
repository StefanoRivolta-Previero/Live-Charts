...
namespace LiveCharts.Wpf
{
  ...
  public static class Extentions
  {
    ...
    public static Point ConvertToChartValues(this Chart chart, Point screenPoint, int axisX = 0, int axisY = 0)
    {
      if (chart.Model == null 
        || chart.AxisX == null 
        || chart.AxisX.Any(x => x.Model == null)
        || chart.AxisY == null
        || chart.AxisY.Any(y => y.Model == null)) return new Point();

      var XX = chart.AxisX[axisX].Model.EvaluatesUnitWidth
              ? ChartFunctions.GetUnitWidth(AxisOrientation.X, chart.Model, axisX) / 2
              : 0;

      var YY = chart.AxisY[axisY].Model.EvaluatesUnitWidth
              ? ChartFunctions.GetUnitWidth(AxisOrientation.Y, chart.Model, axisY) / 2
              : 0;

      var uw = new CorePoint(XX , YY );
